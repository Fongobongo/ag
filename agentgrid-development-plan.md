...
- [ ] Lost step recovery: `lost` step → policy retry/block, без auto-retry side-effectful steps
- [ ] Distributed integration flow: при доступном shared Git remote — immutable worker refs; без shared remote — content-addressed patch/bundle artifact. Не предполагать, что локальные ветки видны другим nodes
- [ ] Conflict policy: integrator не делает silent overwrite; merge conflict → отдельный repair step с лимитом
- [ ] E2E на двух контейнерах: роли одного workflow на разных nodes
- [ ] E2E на двух физических хостах: тот же manifest без изменений
- [ ] Failure injection: потеря worker node посреди run → понятный `lost/blocked`, не зависание
- [ ] ACP plan projection: внешний клиент видит роли/steps/nodes/verdicts live
- [ ] Выпустить тег `v0.3.0`
